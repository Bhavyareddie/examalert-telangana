...
export default function ExamFiltersPanel({ filters, onChange }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 space-y-6">
      ...
      <div>
        ...
        <div className="space-y-1.5">
          <button onClick={() => update('category', undefined)}
            className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${!filters.category ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}>
            All Categories
          </button>
          {EXAM_CATEGORIES.map(cat => (
            <button key={cat} onClick={() => update('category', cat)}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${filters.category === cat ? 'bg-blue-600 text-white' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}>
      ...
      {/* Qualification */}
      <div>
        <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 block">Qualification</label>
      ...
      <div>
        ...
        <select value={filters.sort || 'application_end'} onChange={e => update('sort', e.target.value)}
          ...
          <option value="total_vacancies">Vacancies</option>
